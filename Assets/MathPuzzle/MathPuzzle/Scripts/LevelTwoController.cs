...
public class LevelTwoController : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (markerlList.Count != 0) {
			...
			for (int i = 0; i < markerlList.Count; i++) {
				...
				} else if (isCollision && markerlList [i - 1].ImageTargetObject.transform.Find ("Quad")) {
				...
				if (quadObject != null && markerlList [i].ImageTargetObject.transform.Find ("Quad") && i != 1) {
					
					Destroy (markerlList [i].ImageTargetObject.transform.Find ("Quad").gameObject);

				} else if (!markerlList [i].ImageTargetObject.transform.Find ("Quad") && i == 1) {
					CreateQuad ();
				}
	...
	{
		SortList ();
		if (quadObject == null && markerlList.Count > 1) {
			quadObject = GameObject.CreatePrimitive (PrimitiveType.Quad);
			quadObject.transform.parent = markerlList [1].ImageTargetObject.transform;
			quadObject.transform.localPosition = new Vector3 (0.6f, 0.05f, 0f);
			...
			quadObject.GetComponent<MeshRenderer> ().material = greenMaterial;
	...
	{
		for (int i = 0; i < markerlList.Count; i++) {
			Debug.Log ("marker pos " + markerlList [i].ImageTargetObject.transform.position + "  name " + markerlList [i].ImageTargetObject.name);
		}
	}
}
